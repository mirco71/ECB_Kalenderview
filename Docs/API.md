...
|---|---|---|
| GET | `/api/events?start=ISO&end=ISO` | Termine im Zeitraum (überlappend). Response: `{ erfolg, kalenderName, termine[], startStunde, endStunde }` |
...
| GET | `/api/stats?start=ISO&end=ISO&category_ids=5,7,8` | Abrechnung: Anzahl + Gesamtdauer je Kategorie im Zeitraum, optional nach Titel aufgeschlüsselt (siehe `group_by_title` in [DATABASE.md](DATABASE.md)). Response: `{ erfolg, zeitraum, termineGesamt, gruppen[], gesamt }` |
